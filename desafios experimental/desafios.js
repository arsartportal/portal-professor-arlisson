  ...
  onAuthStateChanged(auth, async(user)=>{
    if(!user){
      document.getElementById("pergunta").innerText = "Faça login para responder ao desafio diário!";
      return;
    }

    
const refUser = doc(db,"usuarios",user.uid);
let snapUser = await getDoc(refUser);

if(snapUser.exists()){
  const dados = snapUser.data();

  if(dados.xpSemana === undefined){
    await updateDoc(refUser,{ xpSemana: 0 });
  }

  const semanaAtual = getSemanaAtual();

  if(dados.semanaRef !== semanaAtual){
  ...
  // 🔥 ATUALIZA SNAP (ESSENCIAL)
  snapUser = await getDoc(refUser);
}

    const ultimoQuiz = snapUser.data()?.ultimoQuizDiario;
    const hoje = getHoje();

    await carregarPergunta(user);
    await carregarStreak(user);
    await iniciarMissaoSemanal(user);
    await carregarRewardDiario(user);
    await observarSP();
  ...
  async function carregarPergunta(user){
  ...
  const hoje = getHoje();
    ...
    const index = Math.abs(getDiaAno()) % bancoQuestoes.length;
  perguntaAtual = bancoQuestoes[index];

  if(ultimoQuiz?.data === hoje){
  ...
  const streakAtual = userSnap.data()?.streakAtual || 0;

  // calcula XP da próxima resposta
  const xpPreview = calcularXPPreview(streakAtual);

  document.getElementById("xpReward").innerText =
    `⚡ +${xpPreview} XP`;

  document.getElementById("materia").innerText = perguntaAtual.materia;
  ...
  if(ultimoQuiz?.data === hoje){
    ...
    btn.onclick = ()=>{
    if(respondeu || ultimoQuiz?.data === hoje){
    btn.disabled = true;
  }
  ...
  if(!user || escolha === null || respondeu) return;
  ...
  let streakAtual = userSnap.data()?.streakAtual || 0;

  const eventoSnap = await getDoc(doc(db,"config","eventos"));
  const xpDobro = eventoSnap.exists() && eventoSnap.data().xp_dobro;
  ...
  if(escolha === perguntaAtual.correta){
    ...
    const XP_BASE = 20;
    const bonus = (novoStreak - 1) * 5;
    xp = XP_BASE + bonus;

    resultado = "acertou";

    if(tempo <= 5) xp += 5;
    else if(tempo <= 10) xp += 3;
  ...
  await runTransaction(db, async (transaction) => {
    ...
    // 🚫 BLOQUEIO TOTAL
    if(dados.ultimoQuizDiario?.data === hoje){
      throw "Já respondeu hoje!";
    }

    let streakAtual = dados.streakAtual || 0;
    let maiorStreak = dados.maiorStreak || 0;

    streakAtual = resultado === "acertou" ? streakAtual + 1 : 0;
...
transaction.update(ref, {
  ...
  function bloquear(resultado){

    const status = document.getElementById("status");
    const feedback = document.getElementById("feedbackVisual");

    if(resultado === "acertou"){
      status.innerText = "🎉 Acertou!";
      feedback.innerText = "🔥 PERFEITO!";
      feedback.className = "feedback-visual acertou";
    } else {
      status.innerText = "❌ Errou";
      feedback.innerText = "💀 Vacilou...";
      feedback.className = "feedback-visual errou";
    }
  }
  ...
  const streak = snap.data()?.streakAtual || 0;

  atualizarUIStreak(streak);
}

async function atualizarStreak(user, resultado){
  ...
  // 🚫 BLOQUEIO CRÍTICO
  if(dados.ultimoQuizDiario?.data === hoje){
    console.warn("⚠️ Streak já contabilizado hoje!");
    return;
  }
  ...
  atualizarUIStreak(streakAtual);
}

  /* =========================
  TOCAR SOM tocarSom("acerto")
  ========================= */

  const sons = {
    acerto: "https://www.myinstants.com/media/sounds/level-up.mp3",
  ...
  function mostrarResultado(){
    ...
    botoes.forEach((btn, i)=>{
      ...
      // resposta errada escolhida → vermelho
      if(i === escolha && i !== perguntaAtual.correta){
  ...
  /* =========================
  MOSTRAR XP PROGRESSIVO NO PREVIEW
  ========================= */

  function calcularXPPreview(streak){
  ...
  function mostrarXPFlutuante(xp, sp){

    const btn = document.getElementById("btnLojaFloat");
  ...
  if(btn){
    ...
    el.innerHTML = `✨ +${xp} XP <br> 🔬 +${sp} SP`;

    el.classList.remove("show");
    void el.offsetWidth; // reset animação
  ...
  function iniciarTimer(){
    ...
    const alvo = getProximaMeiaNoiteBrasil();
  ...
  return;
      ...
      const nextBox = document.getElementById("nextQuestionBox");
      const nextTimer = document.getElementById("nextTimer");

      if(nextBox && nextTimer){
        nextBox.style.display = "block";
        nextTimer.innerText = `${h}h ${m}m ${s}s`;
      }
  ...
  async function iniciarMissaoSemanal(user){
    await garantirMissaoSemanalAtual(user.uid);
    atualizarUI(user);
  }

    async function atualizarUI(user){
    ...
    const snap = await getDoc(doc(db,"usuarios",user.uid));
    const dados = snap.data()?.missaoSemanal;

    if(!dados) return;

    const progresso = dados.desafios || 0;
    const meta = 5;

    texto.innerText = `${progresso} / ${meta}`;
    barra.style.width = (progresso / meta) * 100 + "%";

    // mantém controle do botão
    atualizarBotaoMissao(dados);
  }
  // SE FALTAR, SE FERROU
  function diferencaDias(dataAntiga, hoje){
  ...
  function atualizarUIStreak(streak){

  const el = document.getElementById("streakValor");

  if(!el) return;

  el.innerHTML = `🔥 <strong>${streak}</strong>`;
  ...
  function getProximaMeiaNoiteBrasil(){
    ...
    const meiaNoite = new Date(brasil);
  ...
  if(btn){
    btn.addEventListener("click", coletarRecompensaMissao);
  }
  ...
  async function carregarRewardDiario(user){

    const ref = doc(db,"usuarios",user.uid);
    const snap = await getDoc(ref);

    let data = snap.data()?.dailyReward || {
      ultimoLogin: null,
      dia: 1,
      coletado: false
    };
    ...
    const diasPassados = data.ultimoLogin
      ? diferencaDias(data.ultimoLogin, hoje)
      : 0;

    const msg = document.getElementById("rewardMensagem");
    ...
    let proximoDia = data.dia;

    if(diasPassados === 1){
      proximoDia = data.dia + 1;
    }
    else if(diasPassados > 1){
      proximoDia = 1;

      if(msg){
        msg.innerText = "💔 Sequência perdida! Começando do dia 1";
  ...
  function renderRewardUI(data){
    ...
    rewards.forEach((xp,i)=>{

      const div = document.createElement("div");
      div.classList.add("reward-day");
    ...
    const sp = (data.dia === 7) ? 5 : 1;

  document.getElementById("rewardToday").innerText =
    `+${rewards[data.dia-1]} XP + ${sp} SP`;

    const btn = document.getElementById("btnColetarReward");

    if(data.coletado){
      btn.innerText = "✅ Coletado";
      btn.classList.add("coletado");
      btn.disabled = true;
    } else {
      btn.innerText = "🎁 Coletar recompensa agora";
      btn.classList.remove("coletado");
      btn.disabled = false;
    }
    configurarBotaoReward();
  }

  function configurarBotaoReward(){
    ...
    btn.onclick = async ()=>{
      ...
      const data = snap.data().dailyReward || {
        dia: 1,
        coletado: false,
        ultimoLogin: null
      };
      ...
      // 🔥 bloqueia só se já coletou HOJE
      if(data.coletado && diasPassados === 0){
        return;
      }

      // 🔥 CALCULA O NOVO DIA (CORRETO)
      let novoDia = data.dia;
      ...
      // 🔒 limite ou reset
      if(novoDia > 7){
        novoDia = 1; // loop estilo Duolingo
      }

      const rewards = [10,15,20,25,30,40,100];
      const xp = rewards[novoDia - 1];
      const sp = (novoDia === 7) ? 5 : 1;
      ...
      await updateDoc(ref,{
        dailyReward: {
          ...data,
          dia: novoDia,          // 🔥 AQUI ESTÁ A CHAVE
          coletado: true,
          ultimoLogin: hoje
        }
      });

      // 🔄 atualiza UI já com novo dia
      renderRewardUI({
        ...data,
        dia: novoDia,
        coletado: true
  ...
  function observarSP(){
    ...
    onSnapshot(ref, (snap)=>{
      ...
      const sp = snap.data().sciencePoints || 0;

      const elementos = document.querySelectorAll(".spUsuario");
