/* =====================================================
   MATEMATICA.JS — PORTAL DO PROFESSOR ARLISSON
...
async function carregarTrilhas(uid) {
  ...
  const consulta = usuario.tipo === "professor"
    ? query(
        collection(db, "trilhas_matematica"),
        where("ativo", "==", true),
        orderBy("serie"),
        orderBy("ordem")
      )
  ...
  if (usuario.tipo === "professor") {
    ...
    Object.values(grupos).forEach(g => listaTrilhas.appendChild(g));
...
async function carregarSubniveis(uid, card, trilha) {
  ...
  const niveisRef   = collection(db, "trilhas_matematica", trilha.id, "niveis");
